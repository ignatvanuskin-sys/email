    ...
    try {
      ...
      for (const candidate of pendingLeads) {
        ...
        try {
          ...
          const variants = await prisma.campaignVariant.findMany({ where: { campaignId: id }, orderBy: { sent: "asc" } });
          ...
          await enqueueSend(user.id, "campaign", { campaignId: id, campaignLeadId: candidate.id, leadId: lead.id, emailId: emailRecord.id, variantId, subject: finalSubject, body: finalBody });
          queued++;
        } catch {
