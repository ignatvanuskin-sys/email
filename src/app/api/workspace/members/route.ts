...
export async function POST(req: Request) {
  try { const user = await getApiUser(); if (!user) return unauthorized(); const workspace = await ensureWorkspace(user); const membership = workspace.memberships.find((item) => item.userId === user.id); if (!membership || !roleCan(membership.role, "Admin")) return new Response(JSON.stringify({ error: "Forbidden" }), { status: 403 }); const data = schema.parse(await readJson(req)); const token = randomBytes(32).toString("base64url"); const invitation = await prisma.workspaceInvitation.create({ data: { workspaceId: workspace.id, email: data.email.toLowerCase(), role: data.role, tokenHash: hashInvitationToken(token), invitedById: user.id, expiresAt: new Date(Date.now() + 7 * 86_400_000) } }); await writeAudit(user.id, "workspace.invite_created", "WorkspaceInvitation", invitation.id, { email: data.email, role: data.role }, workspace.id); return ok({ invitation: { id: invitation.id, email: invitation.email, role: invitation.role, expiresAt: invitation.expiresAt.toISOString() }, token }, 201); } catch (error) { return handleError(error); }
}
