...
export default function HomePage() {
  ...
  return (
    <div>
      ...
      {onboarding && onboarding.percent < 100 && <FadeContent><section style={{ margin: "20px 0" }}><div className="row" style={{ marginBottom: 8 }}><div className="section-label grow" style={{ marginBottom: 0 }}>Начало работы</div><span className="small muted">{onboarding.done}/{onboarding.total} выполнено</span></div><div className="card" style={{ padding: 18 }}><div style={{ height: 8, background: "var(--surface-3)", borderRadius: 8, overflow: "hidden", marginBottom: 14 }}><div style={{ width: `${onboarding.percent}%`, height: "100%", background: "var(--accent)", transition: "width .25s ease" }} /></div><div className="stack" style={{ gap: 8 }}>{onboarding.steps.map((step) => <Link href={step.href} key={step.id} className="row small surface-hover" style={{ padding: 8, textDecoration: "none" }}><span className={`badge ${step.completed ? "green" : "gray"}`}>{step.completed ? "Выполнено" : "Далее"}</span><span className="grow">{step.label}</span></Link>)}</div></div></section></FadeContent>}
